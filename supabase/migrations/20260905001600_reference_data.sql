...
insert into public.device_providers (key, vendor, name, description, integration_type, status, capabilities, config_schema, throttling, verification_status, docs_url, sort_order) values
  ('mock', 'FlowZa', 'Mock device (simulator)', 'Simulated device for development and tests. Supports every capability and can inject latency, failures, duplicates and offline periods.',
    ...
    '{"attendancePull":true,"employeePush":true,"employeePull":true,"employeeDelete":true,"fingerprint":true,"face":true,"card":true,"pin":true,"deviceStatus":true,"remoteRestart":true,"webhooks":true,"devicePush":true,"biometricTemplatePush":false}',
    '{"fields":[{"key":"scenario","label":"Scenario","type":"select","options":["healthy","flaky","offline","slow","duplicates","unknown_employees","large_batches"],"default":"healthy","required":true},{"key":"employeeCount","label":"Simulated employees","type":"number","default":25},{"key":"apiKey","label":"API key (simulated)","type":"password","secret":true,"required":false}]}',

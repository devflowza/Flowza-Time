...
| Jobs | Device pushes, exports and imports are queued with `jobs.enqueue` in the same transaction as the state change. The `authenticated` role cannot execute the `jobs` schema, so the API switches back to its login role (`flowza_api`) for the single enqueue call and restores the RLS role (`apps/api/src/lib/jobs.ts`); a `SECURITY DEFINER app.enqueue_job(...)` wrapper would remove the need for that switch. |

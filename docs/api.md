...
| `GET /orgs/:orgId/devices/unmapped-users` | `device.view` | Device user ids (PINs) with no employee behind them, from `unmatched` raw transactions and from `device_employee_states` rows with a null `employee_id`. Filters `branchId, deviceId, origin (all\|punches\|enrolled), search`. Each row carries `unmatchedPunches`, `firstPunchAt`, `lastPunchAt`, `enrolledOnDevice`, `deviceUserName` (the name the device holds). Punch counts obey RLS: a caller without `attendance.view_raw` sees the enrolled ids with a zero count. |
| `POST /orgs/:orgId/devices/:id/user-links` | `device.sync` (+ `employee.update` for `PROVIDER`, + `attendance.view_raw` to replay) | `linkDeviceUserSchema` `{ deviceUserId, employeeId, scope: DEVICE\|PROVIDER, requeueUnmatched }`. `DEVICE` upserts `device_employee_states` for this device; `PROVIDER` upserts `employee_provider_identities` so every device of that vendor resolves the id. `requeueUnmatched` moves that id's `unmatched` raw rows back to `pending` (≤ 5000, `MAX_LINK_REQUEUE`) and enqueues `NORMALIZE_RAW`. Conflicts (`409`): the employee already holds another id on the device, or another employee already owns the id for that vendor. Audit `device.user_linked`. |
| `DELETE /orgs/:orgId/devices/:id/user-links/:deviceUserId?scope=` | same as above | Clears the per-device mapping (`employee_id → null`, `desired = false`) or deletes the vendor identity. Punches already normalised are untouched. Audit `device.user_unlinked`. |

...
| Worker — reports: every sample layout rendered from a DEMO-COMPANY-shaped fixture, asserted line-by-line in CSV and by structure in the PDF HTML; the Chromium renderer smoke test runs when `CHROMIUM_PATH` is set | Vitest + test database | `apps/worker/src/handlers/reports/*.test.ts`, `apps/worker/src/lib/pdf.test.ts` | CI (`database` job installs Chromium) |
| Web — components/hooks | Vitest + Testing Library (jsdom) | `apps/web/src/**/*.test.tsx` | CI |
| UI E2E — sign-in/redirect/validation, dashboard KPIs, the tenant's dashboard style/layout on the shell, live style preview in Settings, employees list + URL search, devices list, ⌘K search, Arabic/RTL switch, no-membership state | Playwright (Chromium, desktop + tablet) against the production bundle with Supabase Auth + API answered by route handlers (`apps/web/e2e/support/mock-backend.ts`) | `apps/web/e2e/*.spec.ts` (`pnpm --filter @flowza/web run build:e2e && pnpm --filter @flowza/web run test:e2e`) | CI `e2e` job |

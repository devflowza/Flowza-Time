...
import { branchInputSchema, departmentInputSchema, designationInputSchema, structureListQuerySchema, teamInputSchema, updateBranchSchema, updateDepartmentSchema, updateDesignationSchema, updateTeamSchema } from '@flowza/contracts';
...
export function registerStructureRoutes(v1: Hono<AppEnv>, deps: ApiDeps): void {
  ...
  v1.patch('/orgs/:orgId/branches/:id', async (c) => ok(c, await s.updateBranch(deps, actorOf(c, deps), param(c, 'orgId'), param(c, 'id'), await body(c, updateBranchSchema))));
  ...
  v1.patch('/orgs/:orgId/departments/:id', async (c) => ok(c, await s.updateDepartment(deps, actorOf(c, deps), param(c, 'orgId'), param(c, 'id'), await body(c, updateDepartmentSchema))));
  ...
  v1.patch('/orgs/:orgId/designations/:id', async (c) => ok(c, await s.updateDesignation(deps, actorOf(c, deps), param(c, 'orgId'), param(c, 'id'), await body(c, updateDesignationSchema))));

...
const itemClass = (collapsed: boolean) =>
  cn(
    'group relative flex h-9 items-center rounded-md text-[13px] text-sidebar-foreground/80 transition-colors',
    ...
    'aria-[current=page]:bg-white/[0.09] aria-[current=page]:font-medium aria-[current=page]:text-white',
    'before:absolute before:start-0 before:top-1/2 before:h-4 before:w-[3px] before:-translate-y-1/2 before:rounded-e-full before:bg-brand-400 before:opacity-0 before:transition-opacity aria-[current=page]:before:opacity-100',
    collapsed ? 'justify-center px-0' : 'gap-2.5 ps-3 pe-2.5',
  );

const iconClass = 'size-[18px] shrink-0 text-sidebar-foreground/60 transition-colors group-hover:text-white group-aria-[current=page]:text-brand-300';
...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn('hidden shrink-0 flex-col bg-sidebar text-sidebar-foreground transition-[width] duration-200 md:flex', collapsed ? 'w-16' : 'w-60')}
      aria-label="Primary"
    >
      <div className={cn('flex h-14 shrink-0 items-center gap-2.5 border-b border-white/[0.07]', collapsed ? 'justify-center px-0' : 'px-3')}>
        <img src="/favicon.svg" alt="" className="size-7 shrink-0 rounded-lg" />
        {!collapsed ? <span className="truncate text-[15px] font-semibold tracking-tight text-white">{t('app.name')}</span> : null}
      </div>
          ...
          return (
            <div key={i} className={i === 0 ? undefined : 'mt-3'}>
              {section.label && !collapsed ? (
                <p className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[0.09em] text-sidebar-foreground/55">{section.label}</p>
              ) : null}
              {/* A hairline stands in for the heading when collapsed, so the grouping survives without the text. */}
              {section.label && collapsed ? <div className="mx-3 mb-1 border-t border-white/[0.07]" /> : null}
      ...
      <div className="shrink-0 border-t border-white/[0.07] p-2">
        <button
          ...
          onClick={toggle}
          className={cn(
            'flex h-9 w-full items-center gap-2.5 rounded-md text-[13px] text-sidebar-foreground/70 transition-colors hover:bg-white/[0.07] hover:text-white',
